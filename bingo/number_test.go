...
func TestNumberString(t *testing.T) {
	t.Run("ok", func(t *testing.T) {
		for _, test := range okNumberTests {
	...
	t.Run("invalid", func(t *testing.T) {
		for _, test := range invalidNumbers {
			if want, got := "?", test.n.String(); want != got {
				t.Errorf("Number(%v) (%v): wanted %v, got %v", int(test.n), test.name, want, got)
			}
		}
	})
}
...
func TestNumberValid(t *testing.T) {
	t.Run("ok", func(t *testing.T) {
		for _, test := range okNumberTests {
			if want, got := true, test.n.Valid(); want != got {
				t.Errorf("Number(%v): wanted %v, got %v", int(test.n), want, got)
			}
		}
	})
	t.Run("invalid", func(t *testing.T) {
		for _, test := range invalidNumbers {
			if want, got := false, test.n.Valid(); want != got {
				t.Errorf("Number(%v) (%v): wanted %v, got %v", int(test.n), test.name, want, got)
			}
		}
	})
